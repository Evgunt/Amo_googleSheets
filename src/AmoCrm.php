...
class AmoCrmV4Client
{
    ...
    {
        $post = array_key_exists('note_type', $PostFields) ? json_encode([$PostFields]) : json_encode($PostFields);
    ...
    function GETField($custom_fields_values, $id)
    {
        $value = '';
        foreach ($custom_fields_values as $field) {
            if ($field['field_id'] == $id || $field['field_code'] == $id) {
